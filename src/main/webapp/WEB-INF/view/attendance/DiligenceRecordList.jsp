...
<div>
			...
			<tbody>
				<c:forEach var="record" items="${diligenceRecords}">
					<tr>
						...
						<td><button onclick="openDetails(${record.recordId})">관리
								/ 管理</button></td>
						<!-- 관리 버튼 / 管理ボタン -->
					</tr>
				</c:forEach>
	...
	<div class="form-container">
		<h3>근태 기록 입력 / 勤怠記録入力</h3>
		<!-- 입력 폼 제목 / 入力フォームのタイトル -->
		<form action="/attendance/saveDiligenceRecord.do" method="post">
			<div class="form-group">
				<label for="diligenceType">근태 항목 / 勤怠項目</label>
				<!-- 근태 항목 선택 / 勤怠項目の選択 -->
			...
			<div class="form-group">
				<label for="startDate">기간 시작일 / 期間開始日</label>
				<!-- 시작일 선택 / 開始日の選択 -->
				<input type="date" id="startDate" name="startDate" required>
			</div>
			<div class="form-group">
				<label for="endDate">기간 종료일 / 期間終了日</label>
				<!-- 종료일 선택 / 終了日の選択 -->
				<input type="date" id="endDate" name="endDate" required>
			</div>
			<div class="form-group">
				<label for="diligenceDays">근태 일수 / 勤怠日数</label>
				<!-- 근태 일수 입력 / 勤怠日数の入力 -->
				<input type="number" id="diligenceDays" name="diligenceDays"
					required>
			</div>
			<div class="form-group">
				<label for="amount">금액(수당) / 金額(手当)</label>
				<!-- 금액 입력 / 金額の入力 -->
				<input type="number" id="amount" name="amount">
			</div>
			<div class="form-group">
				<label for="remarks">비고 / 備考</label>
				<!-- 비고 입력 / 備考の入力 -->
				<input type="text" id="remarks" name="remarks">
			</div>
			<div class="form-group">
				<button type="submit">저장 / 保存</button>
				<!-- 저장 버튼 / 保存ボタン -->
				<button type="reset">내용 지우기 / 内容を消去</button>
				<!-- 초기화 버튼 / リセットボタン -->
			</div>
		</form>
	</div>
